...
import { Detailed } from '@/components/admin/ojaxi'
...
export default function ItemEditor() {
  ...
  const getItem = async () => {
    ...
    try{
        if(id !== null){
          const getItemInfo = await fetch('/api/ojaxi/user', {
            method: 'POST',
            credentials: 'include',
    ...
    <>
      <header className="flex flex-row justify-center items-center">
        <a className='flex flex-row justify-center items-center w-fit' href='https://zrunva.org'>
          <Image className='m-3' src={logo} alt='zrunva.org' width='70' height='50' style={{borderRadius: '0'}}/>
